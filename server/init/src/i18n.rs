...
fn home_text(l: Lang) -> HomeText {
    match l {
        Lang::En => HomeText {
            ...
            p2p_b: "Every peer holds a direct encrypted link to every other peer. Voice and chat travel only there. One shared room key seals group audio. The app encodes and seals each frame once, then sends it to every peer.",
        ...
        Lang::De => HomeText {
            ...
            p2p_b: "Jeder Teilnehmer hält eine direkte verschlüsselte Verbindung zu jedem anderen. Sprache und Chat laufen ausschließlich dort. Ein gemeinsamer Room-Key sichert die Gruppensprache. Die App kodiert und versiegelt jeden Frame einmal und schickt ihn dann an alle Teilnehmer.",
            srv_tag: "Steuerebene",
        ...
        Lang::It => HomeText {
            ...
            p2p_b: "Ogni partecipante mantiene un collegamento cifrato diretto con ogni altro. Voce e chat viaggiano solo lì. Una chiave di stanza condivisa protegge l'audio di gruppo. L'app codifica e sigilla ogni frame una volta sola, poi lo invia a tutti i partecipanti.",
        ...
        Lang::Es => HomeText {
            ...
            p2p_b: "Cada participante mantiene un enlace cifrado directo con todos los demás. La voz y el chat solo circulan ahí. Una clave de sala compartida protege el audio del grupo. La app codifica y sella cada trama una sola vez, luego la envía a todos los participantes.",
        ...
        Lang::Fr => HomeText {
            ...
            p2p_b: "Chaque participant garde une liaison chiffrée directe avec tous les autres. La voix et le chat n'y circulent que là. Une clé de salon partagée protège l'audio de groupe. L'app encode et scelle chaque trame une seule fois, puis l'envoie à tous les participants.",
...
fn credits(l: Lang) -> String {
    let (tested, concept, ai) = match l {
        Lang::En => ("Tested by", "Concept &amp; programming by", "Yes, we used AI."),
        Lang::De => ("Getestet von", "Konzept &amp; Programmierung von", "Ja, wir haben KI benutzt."),
        Lang::It => ("Testato da", "Ideazione e programmazione di", "Sì, abbiamo usato l'IA."),
        Lang::Es => ("Probado por", "Concepto y programación de", "Sí, usamos IA."),
        Lang::Fr => ("Testé par", "Conception et programmation par", "Oui, nous avons utilisé l'IA."),
    };
    format!(
